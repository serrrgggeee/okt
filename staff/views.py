...
class StaffListView(ListView):
    model = Clipping
    queryset = Clipping.objects.order_by('order').all()

    def render_to_response(self, context, **response_kwargs):
        # Shim to affect the CMS Toolbar only
        if self.request.toolbar and self.request.toolbar.edit_mode:
            menu = self.request.toolbar.get_or_create_menu('main-list-menu', 'Leadership')
...
class StaffDetailView(DetailView):
    ...
    def render_to_response(self, context, **response_kwargs):
        # Shim to affect the CMS Toolbar only
        if self.request.toolbar and self.request.toolbar.edit_mode:
            menu = self.request.toolbar.get_or_create_menu('main-member-menu', self.object.full_name)
