...
from slider.models import SliderMember
...
class Clipping(Sortable):
    ...
    slider = models.ForeignKey('slider.SliderMember',
        blank=True,
        default=None,
        help_text=u'Select a Slider Item. This overrides external links.',
        null=True,
    )
    main = models.ForeignKey('main.Main',
            blank=True,
            default=None,
            help_text=u'Select a Slider Item. This overrides external links.',
            null=True,
    ...
    def get_absolute_url(self):
        if self.download:
            ...
            return self.slider.get_absolute_url()
        elif self.main:
            return self.main.get_absolute_url()
        elif self.external_link:
...
#
# NOTE: A news item can "point to" any number of slider members, but we'd like
# the slider members to decide whether that news item will appear on their
# pages. To facilitate this, when we point to a slider member from a news item,
    ...
    if action == 'post_add':
        for staff in StaffMember.objects.filter(pk__in=pk_set):
            if Clipping.objects.filter(staff=staff, external_link='', news_item=instance).count() == 0:
                #
                # OK, looks like slider doesn't already have this as a
                # clipping, so create one.
                #

                # print('Creating Clipping for %s ...' % slider.full_name)
                clipping = Clipping()
...
def link_staff_to_slider(sender, instance, action, pk_set, **kwargs):
    from staff.models import StaffMember

    if action == 'post_add':
        for staff in StaffMember.objects.filter(pk__in=pk_set):
            if Clipping.objects.filter(staff=staff, external_link='', slider=instance).count() == 0:
                #
                # OK, looks like slider doesn't already have this as a
                # clipping, so create one.
                #
                ...
                clipping = Clipping()
                clipping.staff = staff
                clipping.slider = instance
                clipping.save()

# Listen for signals from NewsItem.related_staff M2M changes
m2m_changed.connect(link_staff_to_slider, sender=SliderMember.related_staff.through)
